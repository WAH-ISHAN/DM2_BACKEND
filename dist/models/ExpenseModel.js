...
class ExpenseModel {
    ...
    static async create(userId, payload) {
        // 1️⃣ Insert row
        await (0, db_1.exec)("INSERT INTO expenses (user_id, txn_date, category, description, amount) " +
    ...
    static async delete(userId, id) {
        await (0, db_1.exec)("DELETE FROM expenses WHERE id = :id AND user_id = :u", { id, u: userId }, { autoCommit: true });
    }
    // --------------------  FIND ONE  --------------------
    static async findById(userId, id) {
        const sql = "SELECT id, TO_CHAR(txn_date,'YYYY-MM-DD') AS \"date\", category, description, amount " +
            "FROM expenses WHERE id = :id AND user_id = :u";
        const r = await (0, db_1.exec)(sql, { id, u: userId });
        const x = r.rows?.[0];
        return x
            ? {
                id: x.ID,
